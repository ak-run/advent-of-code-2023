...
def get_next_num_in_seq(sequences):
    """Get next number in a sequence. Input is a list of sequences up to just zeros"""
    for idx in range(1, len(sequences)):
        next_num = sequences[idx][-1] + sequences[idx-1][-1]
        sequences[idx].append(next_num)
    result = sequences[len(sequences)-1][-1]
    return result
...
for sequence in sequences:
    ...
    result += get_next_num_in_seq(list_of_sequences)
